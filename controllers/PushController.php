...
class PushController extends ActiveController
{
    ...
    public function actionAll() {
        ...
        if (!Yii::$app->user->isGuest) {
            $countries = Push::find()->orderBy(['id' => SORT_DESC])->all();
                ...
                if(isset($_FILES['imageFile'])){
                   ...
                   $img = mt_rand(1000000, 8888888);
                    if(empty($errors)==true) {
                        move_uploaded_file($file_tmp,"img/push/".$img.str_replace(" ", "_", $file_name));
                        return ['img' => "img/push/".$img.str_replace(" ", "_", $file_name)];
                    }else{
                        return ['admin' => false];
                    }
                }
